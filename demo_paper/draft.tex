...
\textbf{Unified Model Interface:} Supporting diverse transformer architectures through consistent APIs. Researchers can easily add new models by implementing a standard interface, with automatic handling of architectural differences (encoder-only vs. encoder-decoder, different tokenization schemes, loss functions).

\textbf{Automated Data Pipeline:} Handles nine vulnerability datasets with standardized preprocessing. Features include reproducible seed-based splitting, automatic dataset downloading, consistent function anonymization, and configurable train/validation/test ratios. Custom datasets will require work to convert to the standard JSONL format, however, there are many script recipes to use as templates for this.

\textbf{Rigorous Evaluation Engine:} Implements threshold optimization, multi-seed statistical testing, and comprehensive metrics. Automatically generates performance reports with confidence intervals, statistical significance tests, and comparative analysis.

\textbf{Experiment Tracking:} Integrated Weights \& Biases support with standalone logging fallback. Provides real-time training monitoring, hyperparameter comparison, and automated results aggregation across multiple seeds.

\section{Tool Capabilities and Usage}

\textbf{Easy Model Integration:} Adding new models requires minimal code changes. The framework handles tokenization differences, loss function selection, and representation extraction automatically based on model type annotations.

\textbf{Dataset Management:} Researchers can evaluate on all supported datasets with a single command, or easily add new datasets by following standardized formatting conventions. The system automatically handles class imbalance reporting and data quality assessment.

\textbf{Reproducible Experiments:} All experiments are fully reproducible through seed control, automated environment logging, and standardized evaluation protocols. Results include statistical significance testing across multiple seeds.

\textbf{Threshold Analysis:} Automatic threshold optimization reveals when models achieve high scores through suboptimal default thresholds rather than genuine performance improvements. The system generates threshold-performance curves and optimal operating point recommendations.
