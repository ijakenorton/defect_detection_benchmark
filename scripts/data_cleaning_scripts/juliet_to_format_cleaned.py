...
def anonymize_function_code(func_code, original_func_name):
    """
    Remove identifying information from function code that could leak the target
    - Replace function names with generic names
    - Remove comments that mention "good", "bad", "flaw", "fix", etc.
    """
    lines = func_code.split('\n')
    anonymized_lines = []
    
    for line in lines:
        # Skip or modify comments that give away the answer
        stripped = line.strip()
        if (stripped.startswith('/*') or stripped.startswith('//') or stripped.startswith('*')):
            # Check if comment contains telltale words
            line_lower = stripped.lower()
            telltale_words = ['flaw', 'fix', 'potential flaw', 'bad', 'good', 'vulnerability', 'secure', 'insecure']
            
            if any(word in line_lower for word in telltale_words):
                # Replace with generic comment or skip
                if 'potential flaw' in line_lower or 'flaw' in line_lower:
                    anonymized_lines.append('    /* Potential issue here */')
                elif 'fix' in line_lower:
                    anonymized_lines.append('    /* Alternative implementation */')
                else:
                    # Skip other revealing comments
                    continue
            else:
                # Keep non-revealing comments
                anonymized_lines.append(line)
        else:
            # For code lines, replace function names but keep the structure
            modified_line = line
            
            # Replace the original function name with a generic one
            if original_func_name in line:
                # Generate a generic function name based on CWE number if present
                import re
                cwe_match = re.search(r'CWE(\d+)', original_func_name)
                if cwe_match:
                    generic_name = f"test_function_{cwe_match.group(1)}"
                else:
                    generic_name = "test_function"
                
                modified_line = line.replace(original_func_name, generic_name)
            
            # Also replace any other function names that contain "good" or "bad"
            words = modified_line.split()
            anonymized_words = []
            for word in words:
                # If word contains good/bad and looks like a function name
                if (('good' in word.lower() or 'bad' in word.lower()) and 
                    ('(' in word or word.endswith('();') or word.endswith('()'))):
                    # Replace with generic name
                    if '(' in word:
                        anonymized_words.append('helper_function(')
                    else:
                        anonymized_words.append('helper_function')
                else:
                    anonymized_words.append(word)
            
            modified_line = ' '.join(anonymized_words)
...
def convert_juliet_directory(root_dir, train_ratio=0.7, test_ratio=0.2, val_ratio=0.1):
    ...
    Anonymize function names and comments to prevent cheating
    """
    data = []
    idx = 0
        ...
        for file in files:
            ...
            for func_name, func_code, target in functions:
                if func_code.strip():  # Only add non-empty functions
                    # Anonymize the function code
                    anonymized_code = anonymize_function_code(func_code, func_name)
                    
                    data_point = {
                        "idx": idx,
                        "target": target,
                        "func": anonymized_code.strip()
                    }
