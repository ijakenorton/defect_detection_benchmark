...
sbatch_args() {
    local job_name=$1
    echo "--gpus-per-node=1 --partition=aoraki_gpu_H100 --mem=64gb --job-name=$job_name --time=70:00:00 --output=/projects/sciences/computing/norja159/research/scripts/${job_name}_out/%j.out"
    #echo "--gpus-per-node=1 --partition=aoraki_gpu --mem=64gb --job-name=$job_name --time=70:00:00 --output=/projects/sciences/computing/norja159/research/scripts/${job_name}_out/%j.out"
}
