...
sbatch_args() {
    local job_name=$1
    echo "--gpus-per-node=1 --partition=aoraki_gpu --mem=64gb --job-name=${job_name}_${out_suffix} --time=20:00:00 --output=/projects/sciences/computing/norja159/research/scripts/${job_name}_out/${model_type}_%j.out"
}
